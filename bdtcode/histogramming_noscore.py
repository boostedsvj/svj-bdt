...
def get_scores(rootfile, dataset_name=''):
    '''
    ...
    if cutflow['preselection'] == 0:
        ...
        d = {k : np.array([]) for k in ['mt', 'rt', 'pt', 'energy', 'met', 'phi', 'eta', 'mass', 'metphi', 'lj_pt', 'lj_phi', 'lj_eta', 
...
def dump_score_npz(rootfile, outfile, dataset_name=''):
    '''    
    Dumps events that pass the preselection to a .npz file.
    '''
    d = get_scores(rootfile, dataset_name)
    del d['wtime']
    print(f'Dumping {len(d["mt"])} events from {rootfile} to {outfile}')
